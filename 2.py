...
class Dataset(object):
    def __init__(self, split):
        self.all_videos =glob(args.data_root+'/*')    # 这个获取全部路径下的文件夹. 这些文件夹就是我们最初始提供的视频名字. 每一个文件夹下面是他的帧图片和wav
...
#============他这里面音频取的是随机一个视频?????????
            vidname_mel = self.all_videos[idx_mel] #=====这个地方参考wav2lip的haul应该是改成.下面这行.
            ...
            window[:, :, window.shape[2]//2:-mask_margin, mask_margin:-mask_margin] = -1 #======人脸下班部分用-1填充.
...
def run():
    ...
    if args.syncnet_checkpoint_path is not None:
     ...
     # Model
    model = Wav2Lip(audio_encoder).to(device) # 让这两个模型共享audio_encoder
    model.freeze_audio_encoder()
    disc = Wav2Lip_disc_qual().to(device)

    print('total wav2lip trainable params {}'.format(sum(p.numel() for p in model.parameters())))
    print('total disc trainable params {}'.format(sum(p.numel() for p in disc.parameters())))
    print('total syncnet  params {}'.format(sum(p.numel() for p in syncnet.parameters())))
